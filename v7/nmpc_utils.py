...
def nmpc_solver(robot_state, x_ref, dynamic_obstacles, static_obstacles):
    ...
    # --- Dynamics constraints (Unchanged) ---
    for k in range(const.N):
    ...
    for k in range(const.N + 1):
        ...
        for i, obs in enumerate(dynamic_obstacles):
            ...
            # --- MODIFIED: CBF with Uncertainty ---
            effective_radius = obs.radius
            if const.NOISY_OBSTACLES:
                # Add uncertainty radius. We use sqrt of trace as an approximation of the ellipse size.
                cov = obs.predicted_cov[k]
                uncertainty_radius = const.SIGMA_BOUND * ca.sqrt(cov[0,0] + cov[1,1])
    ...
    opti.subject_to(opti.bounded(0, X[0, :], const.X_LIM)); opti.subject_to(opti.bounded(0, X[1, :], const.Y_LIM))
