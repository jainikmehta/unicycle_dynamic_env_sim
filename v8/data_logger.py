...
class DataLogger:
    ...
    def log_timestep(self, t, robot, goal, dynamic_obstacles):
        ...
        self.file.write(f"robot_true_state: ({robot.state[0]:.2f}, {robot.state[1]:.2f}, {robot.state[2]:.2f})\n")
        self.file.write(f"goal_state: ({goal.state[0]:.2f}, {goal.state[1]:.2f})\n")
        ...
        for i, obs in enumerate(sorted_obstacles):
            self.file.write(f"obstacle_{i+1}_measured_state: ({obs.measured_state[0]:.2f}, {obs.measured_state[1]:.2f}, {obs.measured_state[2]:.2f})\n")
