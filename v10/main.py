...
class IntermediateGoal:
    ...
    def is_valid(self, dynamic_obstacles):
        if self.point is None:
            return False
        for obs in dynamic_obstacles:
            for i in range(const.N + 1):
                obs_pos = obs.predicted_path[:, i]
                dist = np.linalg.norm(self.point - obs_pos)
                if dist < const.INTERMEDIATE_GOAL_COLLISION_RADIUS:
                    return False
        return True

    def update(self, robot_state, rrt_path, dynamic_obstacles):
        if rrt_path is None:
            self.point = None
            return
        ...
        dist_along_path = 0
        while dist_along_path < const.INTERMEDIATE_GOAL_LOOKAHEAD_DISTANCE and lookahead_idx + 1 < len(rrt_path):
            dist_along_path += np.linalg.norm(rrt_path[lookahead_idx+1] - rrt_path[lookahead_idx])
            lookahead_idx += 1
        
        candidate_point = rrt_path[lookahead_idx]
        is_safe = True
        for obs in dynamic_obstacles:
            for i in range(const.N + 1):
                obs_pos = obs.predicted_path[:, i]
                dist = np.linalg.norm(candidate_point - obs_pos)
                if dist < const.INTERMEDIATE_GOAL_COLLISION_RADIUS:
                    is_safe = False
                    break
            if not is_safe:
...
def setup_environment(grid):
    ...
    for _ in range(100):
        ...
        if robot_pos is not None:
            ...
            if (np.linalg.norm([rx-gx,ry-gy]) >= const.MIN_START_GOAL_DIST and
                check_min_h_values(temp_robot_state, dynamic_obstacles, static_obstacles) > const.MIN_SPAWN_H_VALUE):
...
def generate_goal_oriented_reference(robot_state, intermediate_goal):
    ref_path = np.zeros((2, const.N + 1))
    if intermediate_goal is None:
        for i in range(const.N + 1):
            ref_path[:, i] = robot_state[:2]
        return ref_path
        
    direction = (intermediate_goal - robot_state[:2])
    distance = np.linalg.norm(direction)
    direction /= distance if distance > 0 else 1

    for i in range(const.N + 1):
        step_dist = min(distance, i * const.MAX_LINEAR_VEL * const.DT)
        ref_path[:, i] = robot_state[:2] + step_dist * direction
    return ref_path
...
def check_min_h_values(robot_state, dynamic_obstacles, static_obstacles):
    ...
    for obs in dynamic_obstacles:
        ...
        uncertainty_radius = obs.sigma_bounds[0] * np.sqrt(np.trace(obs.cov)) if hasattr(obs, 'sigma_bounds') and len(obs.sigma_bounds) > 0 else 0
...
if __name__ == "__main__":
    ...
    try:
        ...
        intermediate_goal = IntermediateGoal()
        intermediate_goal.update(robot.state, rrt_path, dynamic_obstacles)
        ...
        for t in range(const.MAX_SIM_STEPS):
            ...
            dist_to_intermediate_goal = np.linalg.norm(robot.state[:2] - intermediate_goal.point) if intermediate_goal.point is not None else float('inf')
            
            if not intermediate_goal.is_valid(dynamic_obstacles) or dist_to_intermediate_goal < const.INTERMEDIATE_GOAL_REACH_THRESHOLD:
                print("Updating intermediate goal...")
                intermediate_goal.update(robot.state, rrt_path, dynamic_obstacles)
                if intermediate_goal.point is None:
                    print("Could not find a safe intermediate goal. Re-planning RRT*...")
                    rrt_path = plan_rrt(robot, goal, static_obstacles, dynamic_obstacles)
                    if rrt_path is None:
            ...
            x_ref = generate_goal_oriented_reference(robot.state, intermediate_goal.point)
            ...
            plot_environment(robot, goal, static_obstacles, dynamic_obstacles,
                             ...
                             h_dyn_history, h_stat_history, intermediate_goal.point, save_path=frame_path)
